...
CI runs quality gates, Linux/macOS builds, canonical example E2E, Docker integration fixtures, nasty integration matrix checks, dogfood config validation, deploy/release fixtures, and a snapshot release smoke. Tag releases publish darwin/linux artifacts for amd64 and arm64 after the same archive smoke. Release artifacts include `checksums.txt`; smoke verification hashes every archive and checks the packaged binary's version provenance. Windows and Homebrew are intentionally out of scope for now.
