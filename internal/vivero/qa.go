...
func (a *App) QAPlanWithTarget(previewID, scopeName, target string) (map[string]any, error) {
	...
	return map[string]any{
		...
		"commands": map[string]any{
			...
			"record":      qaCommandWithTarget(fmt.Sprintf("vivero preview qa record %s --scope %s --json --no-input --quiet", previewRef, selectedScope), target),
		...
		"agentInstructions": []string{
			...
			"Screenshots, QA reports, and recordings default to local preview URLs for speed; pass --target public only for explicit public-tunnel validation.",
