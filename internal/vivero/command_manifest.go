...
func commandManifests() []CommandManifest {
	...
	commands := []CommandManifest{
		...
		manifest([]string{"exec"}, "run a command in a preview service", "vivero exec preview:my-preview web --timeout 10m --json --no-input -- npm test", true, "stable", append(global, CommandFlag{Name: "--timeout", ValueName: "DURATION", Description: "command timeout", Default: "30m"}), []CommandArg{previewTargetArg, {Name: "service", Required: true}, {Name: "command", Required: true}}, map[string]any{"returns": "stdout, stderr, exitCode, and timedOut", "targetRefs": []string{"preview:<id>"}}),
		...
		manifest([]string{"qa", "context"}, "alias for QA plan", "vivero qa context preview:my-preview --scope all --target local --json --no-input", true, "stable", append(global, CommandFlag{Name: "--scope", ValueName: "NAME|all", Description: "QA scope"}, CommandFlag{Name: "--target", ValueName: "local|public|origin", Description: "QA target", Default: "local"}), []CommandArg{previewTargetArg}, withTargetRefs(qaSchema())),
		...
		manifest([]string{"qa", "record"}, "record QA walkthrough video", "vivero qa record preview:my-preview --scope smoke --target local --color-scheme dark --json --no-input", true, "stable", append(global, CommandFlag{Name: "--scope", ValueName: "NAME|all", Description: "QA scope"}, CommandFlag{Name: "--target", ValueName: "local|public|origin", Description: "QA target", Default: "local"}, CommandFlag{Name: "--output-dir", ValueName: "DIR", Description: "write video artifacts under this directory"}, CommandFlag{Name: "--color-scheme", ValueName: "light|dark", Description: "recording color scheme"}, CommandFlag{Name: "--storage-state", ValueName: "PATH", Description: "Playwright storage state for authenticated evidence"}, CommandFlag{Name: "--format", ValueName: "mp4|webm", Description: "recording format", Default: "mp4"}, CommandFlag{Name: "--slow-mo-ms", ValueName: "MS", Description: "slow Playwright actions for readable recordings"}, CommandFlag{Name: "--wait-ms", ValueName: "MS", Description: "wait after each action before continuing", Default: "1500"}, CommandFlag{Name: "--width", ValueName: "PX", Description: "recording viewport width", Default: "1280"}, CommandFlag{Name: "--height", ValueName: "PX", Description: "recording viewport height", Default: "800"}, CommandFlag{Name: "--device-scale-factor", ValueName: "N", Description: "recording device scale factor", Default: "1"}), []CommandArg{previewTargetArg}, withTargetRefs(qaSchema())),
		withSideEffects(manifest([]string{"qa", "final"}, "run final QA proof and write handoff artifact", "vivero qa final preview:my-preview --scope smoke --json --no-input", true, "stable", append(global, CommandFlag{Name: "--scope", ValueName: "NAME|all", Description: "QA scope"}, CommandFlag{Name: "--target", ValueName: "local|public|origin", Description: "QA target", Default: "local"}, CommandFlag{Name: "--no-screenshots", Description: "skip screenshot capture"}, CommandFlag{Name: "--no-record", Description: "skip walkthrough video recording"}, CommandFlag{Name: "--include-evidence", ValueName: "PATH", Description: "include an ad-hoc evidence flow JSON result in final proof; repeatable"}, CommandFlag{Name: "--color-scheme", ValueName: "light|dark", Description: "recording color scheme"}, CommandFlag{Name: "--storage-state", ValueName: "PATH", Description: "Playwright storage state for authenticated evidence"}, CommandFlag{Name: "--format", ValueName: "mp4|webm", Description: "recording format", Default: "mp4"}, CommandFlag{Name: "--slow-mo-ms", ValueName: "MS", Description: "slow Playwright actions for readable recordings"}, CommandFlag{Name: "--wait-ms", ValueName: "MS", Description: "wait after each action before continuing", Default: "1500"}, CommandFlag{Name: "--width", ValueName: "PX", Description: "recording viewport width", Default: "1280"}, CommandFlag{Name: "--height", ValueName: "PX", Description: "recording viewport height", Default: "800"}, CommandFlag{Name: "--device-scale-factor", ValueName: "N", Description: "recording device scale factor", Default: "1"}), []CommandArg{previewTargetArg}, withTargetRefs(qaSchema())), true, false, false),
...
func configDoctorSchema() map[string]any {
	return map[string]any{
		...
		"findingCodes": []string{
			...
			"compose-health-timeout-missing",
			"compose-file-unverified",
			"compose-source-invalid",
			"compose-file-invalid",
			"compose-file-missing",
			"compose-inspection-unavailable",
			"compose-config-invalid",
			"compose-service-missing",
			"compose-services-omitted",
			"compose-host-ports-stripped",
...
func evidenceFlowSchema() map[string]any {
	return map[string]any{
		...
		"defaults":   map[string]any{"target": "local", "format": "mp4", "width": 1280, "height": 800, "deviceScaleFactor": 1, "screenshots": true, "console": true, "video": false, "network": false, "pointer": true},
		"stepsFile": map[string]any{
			...
			"record":         "video/screenshots/console/network booleans plus pointer boolean; pointer defaults true for video evidence and draws visible cursor/click feedback",
			"actions":        []string{"visit/goto endpoint", "click locator", "fill locator+value", "press key", "scroll direction/object", "wait/waitMs", "waitForSelector", "expectText", "expectNoText", "expectSelector", "expectNoSelector", "expectUrl", "expectUrlNot", "screenshot"},
...
func qaSchema() map[string]any {
	return map[string]any{
		"usage":       "vivero preview qa <plan|context|run|final|report> <preview-id|preview:<id>> [--scope <name|all>] [--public|--target local|public|origin] --json --no-input; vivero preview qa record <preview-id|preview:<id>> [--scope <name|all>] [--target local|public|origin] [--output-dir <dir>] [--storage-state <path>] --json --no-input",
		...
		"record":      "records declared QA flows through the selected QA target URL; use --target public only when validating the public tunnel",
		"final":       "runs qa run plus a walkthrough recording, startup diagnosis, optional ad-hoc evidence flow result includes, and writes final.json with URL, smoke status, verified screenshots/videos, report path, and artifact paths",
		"recordOptions": map[string]any{
			"target":       "local|public|origin, default local; controls QA plan URLs used by recordings",
			"outputDir":    "optional directory for video artifacts; generated evidence.recordings.commands set this to the QA video artifact directory",
			...
			"waitMs":       "default 1500 for readable, non-microscopic recordings; pass 0 explicitly to disable waits",
		},
		"finalOptions": map[string]any{
			"includeEvidence": "repeatable path to an ad-hoc evidence flow JSON result; media paths are merged into final proof and verified as non-empty files",
		},
