...
func commandManifests() []CommandManifest {
	...
	commands := []CommandManifest{
		...
		manifest([]string{"qa", "record"}, "record QA walkthrough video", "vivero qa record preview:my-preview --scope smoke --target local --color-scheme dark --json --no-input", true, "stable", append(global, CommandFlag{Name: "--scope", ValueName: "NAME|all", Description: "QA scope"}, CommandFlag{Name: "--target", ValueName: "local|public|origin", Description: "QA target", Default: "local"}, CommandFlag{Name: "--color-scheme", ValueName: "light|dark", Description: "recording color scheme"}, CommandFlag{Name: "--storage-state", ValueName: "PATH", Description: "Playwright storage state for authenticated evidence"}), []CommandArg{previewTargetArg}, withTargetRefs(qaSchema())),
...
func qaSchema() map[string]any {
	return map[string]any{
		"usage":       "vivero preview qa <plan|context|run|final|report> <preview-id|preview:<id>> [--scope <name|all>] [--public|--target local|public|origin] --json --no-input; vivero preview qa record <preview-id|preview:<id>> [--scope <name|all>] [--target local|public|origin] [--storage-state <path>] --json --no-input",
