...
func TestProjectConfigProfileAppliesServiceEnvOverlay(t *testing.T) {
	cfg := ProjectConfig{
		...
		Profiles: map[string]ProfileConfig{
			"product-a": {
				Services: []string{"helper-web", "product-a-web"},
				ServiceEnv: map[string]map[string]string{
					"helper-web": {
						"HOST_PRODUCT":  "product-a",
						"PRODUCT_A_URL": "http://product-a-web:3310",
					},
				},
			},
		},
	...
	profiled, active, err := projectConfigForRequestedProfile(cfg, "product-a")
	if err != nil {
		t.Fatal(err)
	}
	if active != "product-a" {
	...
	if helperEnv["HOST_PRODUCT"] != "product-a" {
		t.Fatalf("profile env should override service env: %#v", helperEnv)
	}
	if helperEnv["PRODUCT_A_URL"] != "http://product-a-web:3310" {
