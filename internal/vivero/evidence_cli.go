...
func (a *App) runEvidence(args []string, stdout, stderr io.Writer, jsonOut bool) int {
	...
	case "flow":
		...
		if evidenceTargetKind(pos[0]) == "release" {
			return errOut(stderr, jsonOut, newCLIError("unsupported_target", "release targets do not support browser evidence flows", "Use `vivero evidence flow preview:<id> --steps-file flow.json`", map[string]string{"command": "evidence flow", "target": pos[0]}))
		}
